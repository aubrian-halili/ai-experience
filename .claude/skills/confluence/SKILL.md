...
variable. Substitute its value wherever `${ATLASSIAN_HOST}` appears below; if it is unset,
say so and ask for the tenant rather than guessing. Prefer `--json` on all acli commands.
